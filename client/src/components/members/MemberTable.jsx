...
function MemberTable({
  ...
  const handleDelete = async (member) => {
    ...
    const result = await confirmAlert(
      "Delete Member?",
      `${member.name} will be removed permanently`
    );
          ...
          <tbody>
            {members.map((m) => (
              ...
              >
                ...
                <td className="p-4">
                  <span
                    className={`px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider ${statusColor(m.status)}`}
                  >
                    {m.status}
                  </span>
                </td>
        ...
        {members.map((m) => (
          ...
          >
            <div className="flex justify-between items-start">
              ...
              <span
                className={`px-2 py-1 rounded-full text-[10px] font-bold uppercase ${statusColor(m.status)}`}
              >
                {m.status}
              </span>
            </div>
            ...
            <div className="flex justify-between items-center text-sm">
              ...
              <span className="text-gray-500">
                Plan:
                <span className="text-gray-900 capitalize font-medium ml-1">
                  {m.plan}
                </span>
              </span>
